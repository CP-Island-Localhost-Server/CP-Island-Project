...
//	[DllImport("__Internal")] static extern int dlclose(IntPtr handle);
	...
	private static void DataCallback(int reqId, IntPtr data, int size) {
		...
		public static void DestroyAllBrowsers()
		{
			try
			{
				int num = zfb_numBrowsers();
				Debug.Log("[BrowserNative] DestroyAllBrowsers() - Num browsers before destroy: " + num);
				if (num <= 0)
				{
					Debug.LogWarning("BrowserNative] DestroyAllBrowsers() - No browsers to destroy. Returning without calling zfb_destroyAllBrowsers()");
				}
				else
				{
					zfb_destroyAllBrowsers();
					int num2 = zfb_numBrowsers();
					Debug.Log("[BrowserNative] DestroyAllBrowsers() - Num browsers after destroy: " + num2);
					if (num2 != 0)
					{
						throw new Exception("Not all browsers were destroyed. Num remaining: " + num2);
					}
				}
			}
			catch (Exception ex)
			{
				Debug.LogError("[BrowserNative] DestroyAllBrowsers() - Exception thrown:\n" + ex.Message + ex.StackTrace);
				throw;
			}
		}

	...
	public class Pkcs12Store
	{
		...
		public void Load(Stream input, char[] password)
		{
			...
			if (password != null && pfx.MacData != null)
			{
				...
				byte[] a = CalculatePbeMac(algorithmID.Algorithm, salt, intValue, password, false, octets);
				...
				if (!Arrays.ConstantTimeAreEqual(a, digest))
				{
					...
					a = CalculatePbeMac(algorithmID.Algorithm, salt, intValue, password, true, octets);
					if (!Arrays.ConstantTimeAreEqual(a, digest))
					{
						throw new IOException("PKCS12 key store MAC invalid - wrong password or corrupted file.");
					}
			...
			if (authSafe.ContentType.Equals(PkcsObjectIdentifiers.Data))
			{
				...
				foreach (ContentInfo contentInfo2 in array)
				{
					...
					else if (contentType.Equals(PkcsObjectIdentifiers.EncryptedData) && password != null)
					{
						...
						array2 = CryptPbeData(false, instance.EncryptionAlgorithm, password, wrongPkcs12Zero, instance.Content.GetOctets());
					}
					if (array2 == null)
					{
						continue;
		...
		public void Save(Stream stream, char[] password, SecureRandom random)
		{
			...
			if (password != null)
			{
				...
				byte[] digest = CalculatePbeMac(OiwObjectIdentifiers.IdSha1, array3, 1024, password, false, encoded);

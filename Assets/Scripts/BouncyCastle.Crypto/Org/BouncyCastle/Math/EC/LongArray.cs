	...
	internal class LongArray
	{
		...
		private static long AddShiftedUp(long[] x, int xOff, long[] y, int yOff, int count, int shift)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				array2[num4] = array[(long)intPtr] ^ ((num3 << shift) | num2);
				num2 = (long)((ulong)num3 >> num);
			}
			return num2;
		}
		...
		private static long AddShiftedDown(long[] x, int xOff, long[] y, int yOff, int count, int shift)
		{
			...
			while (--num3 >= 0)
			{
				...
				int num5 = xOff + num3;
				IntPtr intPtr = (IntPtr)num5;
				array2[num5] = array[(long)intPtr] ^ ((long)((ulong)num4 >> shift) | num2);
				num2 = num4 << num;
			}
			return num2;
		}
		...
		private static void Add(long[] x, int xOff, long[] y, int yOff, int count)
		{
			for (int i = 0; i < count; i++)
			{
				...
				array2[num] = array[(long)intPtr] ^ y[yOff + i];
			}
		}
		...
		private static void AddBoth(long[] x, int xOff, long[] y1, int y1Off, long[] y2, int y2Off, int count)
		{
			for (int i = 0; i < count; i++)
			{
				...
				int num = xOff + i;
				IntPtr intPtr = (IntPtr)num;
				array2[num] = array[(long)intPtr] ^ (y1[y1Off + i] ^ y2[y2Off + i]);
			}
		}
		...
		private static void Distribute(long[] x, int src, int dst1, int dst2, int count)
		{
			for (int i = 0; i < count; i++)
			{
				...
				array2[num2] = array[(long)intPtr] ^ num;
				long[] array3 = (array = x);
				int num3 = dst2 + i;
				intPtr = (IntPtr)num3;
				array3[num3] = array[(long)intPtr] ^ num;
			}
		}
		...
		private static void FlipWord(long[] buf, int off, int bit, long word)
		{
			...
			if (num2 == 0)
			{
				...
				int num3 = num;
				intPtr = (IntPtr)num3;
				array2[num3] = array[(long)intPtr] ^ word;
				return;
			}
			long[] array3 = (array = buf);
			int num4 = num;
			intPtr = (IntPtr)num4;
			array3[num4] = array[(long)intPtr] ^ (word << num2);
			...
			if (word != 0)
			{
				...
				int num5 = ++num;
				intPtr = (IntPtr)num5;
				array4[num5] = array[(long)intPtr] ^ word;
			}
		}
			...
			while ((a = (long)((ulong)a >> 1)) != 0)
			{
				if ((a & 1) != 0)
				{
					...
					if (num2 != 0)
					{
						...
						array2[num3] = array[(long)intPtr] ^ num2;
					}
				}
				num++;
			}
		...
		private static void ReduceWordWise(long[] buf, int off, int len, int toBit, int m, int[] ks)
		{
			...
			if (num4 != 0)
			{
				...
				int num5 = off + num;
				IntPtr intPtr = (IntPtr)num5;
				array2[num5] = array[(long)intPtr] ^ (num4 << num3);

namespace DeviceDB
{
	internal class LongIndexValueType : AbstractFixedIndexValueType<long>
	{
		private readonly byte[] buffer;

		public LongIndexValueType()
		{
			buffer = new byte[8];
		}

		protected override byte[] Serialize(long entryValue)
		{
			...
			buffer[7] = (byte)((ulong)(entryValue & -72057594037927936L) >> 56);
			return buffer;
		}

		protected override long Deserialize(byte[] bytes)
		{
			return (long)(((ulong)(uint)(bytes[4] | (bytes[5] << 8) | (bytes[6] << 16) | (bytes[7] << 24)) << 32) | (uint)(bytes[0] | (bytes[1] << 8) | (bytes[2] << 16) | (bytes[3] << 24)));
		}
	}
}

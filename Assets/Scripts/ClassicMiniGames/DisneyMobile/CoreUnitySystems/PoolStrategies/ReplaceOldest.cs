	...
	public class ReplaceOldest : ObjectPoolFullStrategy
	{
		...
		private GameObject GetOldestObject(List<GameObject> activeInstances)
		{
			...
			for (int i = 0; i < activeInstances.Count; i++)
			{
				...
				{
					Logger.LogFatal(activeInstances[i], "Attempting to replace pooled instance based on lifetime (Oldest), however the game object or prefab is missing the required PooledLifetimeComponent!", Logger.TagFlags.CORE | Logger.TagFlags.MEMORY);
				}
			}
			return result;
		}

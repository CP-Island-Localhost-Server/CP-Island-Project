	...
	public virtual void UpdateScrollbars(bool recalculateBounds)
	{
		...
		if (horizontalScrollBar != null || verticalScrollBar != null)
		{
			...
			if (verticalScrollBar != null && vector2.y > vector.y)
			{
				...
				if (mPanel.clipping == UIDrawCall.Clipping.SoftClip)
				{
					f -= mPanel.clipSoftness.y;
				}
				float contentSize = vector2.y - vector.y;
				float viewSize = f * 2f;
				float x = vector.y;
				float x2 = vector2.y;
				float num2 = finalClipRegion.y - f;
				float num3 = finalClipRegion.y + f;
				x = num2 - x;
				x2 -= num3;
				UpdateScrollbars(verticalScrollBar, x, x2, contentSize, viewSize, true);
			}
		}

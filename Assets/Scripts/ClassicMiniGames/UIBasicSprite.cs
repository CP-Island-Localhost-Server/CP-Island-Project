...
public abstract class UIBasicSprite : UIWidget
{
	...
	private void FilledFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (mFillDirection == FillDirection.Horizontal || mFillDirection == FillDirection.Vertical)
		{
			...
			{
				float num = (drawingUVs.w - drawingUVs.y) * mFillAmount;
				if (mInvert)
				{
		...
		if (mFillAmount < 1f)
		{
			...
			if (mFillDirection == FillDirection.Radial360)
			{
				for (int k = 0; k < 4; k++)
				{
					...
					mTempUVs[2].y = mTempUVs[1].y;
					mTempUVs[3].y = mTempUVs[0].y;
					float value = mInvert ? (mFillAmount * 4f - (float)NGUIMath.RepeatIndex(k + 2, 4)) : (mFillAmount * 4f - (float)(3 - NGUIMath.RepeatIndex(k + 2, 4)));
					if (RadialCut(mTempPos, mTempUVs, Mathf.Clamp01(value), mInvert, NGUIMath.RepeatIndex(k + 2, 4)))
					{
						for (int i = 0; i < 4; i++)
						{
							verts.Add(mTempPos[i]);
			...
			for (int j = 0; j < 3; j++)
			{
				...
				else if (i == 1)
				{
					if ((j == 0 && bottomType == AdvancedType.Tiled) || (j == 2 && topType == AdvancedType.Tiled))
					{
						...
						float y3 = mTempUVs[j].y;
						float num5 = mTempUVs[num2].y;
						for (float num4 = x; num4 < x2; num4 += vector2.x)
						{
							float num7 = num4 + vector2.x;
							float num8 = mTempUVs[num].x;
							if (num7 > x2)
							...
							Fill(verts, uvs, cols, num4, num7, y, y2, x3, num8, y3, num5, drawingColor);
						}
					}
				...
				else if (j == 1)
				{
					if ((i == 0 && leftType == AdvancedType.Tiled) || (i == 2 && rightType == AdvancedType.Tiled))
					{
						...
						for (float num3 = y; num3 < y2; num3 += vector2.y)
						{
							...
							Fill(verts, uvs, cols, x, x2, num3, num6, x3, num8, y3, num5, drawingColor);
						}
					}

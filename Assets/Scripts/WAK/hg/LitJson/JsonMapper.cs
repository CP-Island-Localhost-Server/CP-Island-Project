...
namespace hg.LitJson
{
	public class JsonMapper
	{
		...
		private static void AddObjectMetadata(Type type)
		{
			...
			foreach (PropertyInfo propertyInfo in properties)
			{
				if (propertyInfo.IsDefined(typeof(JsonIgnoreAttribute), true))
				{
					continue;
				}
				if (propertyInfo.Name == "Item")
			...
			foreach (FieldInfo fieldInfo in fields)
			{
				if (!fieldInfo.IsDefined(typeof(JsonIgnoreAttribute), true))
				{
					...
					if (fieldInfo.IsDefined(typeof(JsonPropertyAttribute), true))
					{
						JsonPropertyAttribute[] array = (JsonPropertyAttribute[])fieldInfo.GetCustomAttributes(typeof(JsonPropertyAttribute), true);
						value3.JsonPropertyName = array[0].Name;
						value.Properties.Add(array[0].Name, value3);
					}
					else
					{
		...
		private static void AddTypeProperties(Type type)
		{
			...
			foreach (PropertyInfo propertyInfo in properties)
			{
				if (!propertyInfo.IsDefined(typeof(JsonIgnoreAttribute), true) && !(propertyInfo.Name == "Item"))
				{
					...
					{
						JsonPropertyAttribute[] array = (JsonPropertyAttribute[])propertyInfo.GetCustomAttributes(typeof(JsonPropertyAttribute), true);
						item.JsonPropertyName = array[0].Name;
						list.Add(item);
					}
					else
					{
			...
			foreach (FieldInfo fieldInfo in fields)
			{
				if (!fieldInfo.IsDefined(typeof(JsonIgnoreAttribute), true))
				{
					...
					if (fieldInfo.IsDefined(typeof(JsonPropertyAttribute), true))
					{
						JsonPropertyAttribute[] array2 = (JsonPropertyAttribute[])fieldInfo.GetCustomAttributes(typeof(JsonPropertyAttribute), true);
						item2.JsonPropertyName = array2[0].Name;
						list.Add(item2);
					}
					else
					{
		...
		private static object ReadValue(Type inst_type, JsonReader reader)
		{
			...
			if (reader.Token == JsonToken.Double || reader.Token == JsonToken.Int || reader.Token == JsonToken.Long || reader.Token == JsonToken.String || reader.Token == JsonToken.Boolean)
			{
				...
				if (custom_importers_table.ContainsKey(type2) && custom_importers_table[type2].ContainsKey(inst_type))
				{
			...
			else if (reader.Token == JsonToken.ObjectStart)
			{
				...
				{
					throw new NotSupportedException("JSON failed to map to your model.  Please check the types inside of your models. Message: " + ex.Message);
				}
				while (true)
				{
					reader.Read();
		...
		private static void WriteValue(object obj, JsonWriter writer, bool writer_is_private, int depth)
		{
			...
			else
			{
				...
				foreach (PropertyMetadata item in list)
				{
					if (item.IsField)
					{
						writer.WritePropertyName((item.JsonPropertyName == null) ? item.Info.Name : item.JsonPropertyName);
		...
		public static object ToObject(string json, Type ofType)
		{
			JsonReader reader = new JsonReader(json);
			return ReadValue(ofType, reader);
		}
